...
FEATURE_NAMES = ['radius_mean','texture_mean','perimeter_mean','area_mean','smoothness_mean','compactness_mean','concavity_mean','concave points_mean','symmetry_mean','fractal_dimension_mean','radius_se','texture_se','perimeter_se','area_se','smoothness_se','compactness_se','concavity_se','concave points_se','symmetry_se','fractal_dimension_se','radius_worst','texture_worst','perimeter_worst','area_worst','smoothness_worst'
...
class ID3:
    ...
    def entropy(rows: np.ndarray, labels: np.ndarray):
        ...
        # ====== YOUR CODE: ======
        # entropy = pv * logp + (1-pv)log(1-pv)
        if "M" in counts.keys():
            p_value = counts["M"] / rows.shape[0]
        else:
            p_value = 0
        if p_value in [0,1]:
            return 0
        _entropy = p_value * math.log2(p_value) + (1-p_value)* math.log2(1-p_value)
        return -_entropy
    ...
    def info_gain(self, left, left_labels, right, right_labels, current_info_gain=None):
        ...
        info_gain_value = current_info_gain - left.shape[0]/count * left_entropy - right.shape[0]/count * right_entropy
        # ========================
    ...
    def partition(self, rows, labels, question: Question, current_uncertainty):
        ...
        gain = self.info_gain(np.array(true_rows), np.array(true_labels), np.array(false_rows), np.array(false_labels), current_uncertainty)
        # ========================
        return gain, np.array(true_rows), np.array(true_labels), np.array(false_rows), np.array(false_labels)

    def create_feature_splits(self, feature_col):
        splits = np.copy(feature_col)
        splits = np.sort(feature_col)
        new_splits = []
        for i in range(splits.shape[0] - 1):
            new_splits.append((splits[i] + splits[i+1])/2)
        return new_splits
    ...
    def find_best_split(self, rows, labels):
        ...
        current_uncertainty = self.entropy(np.array(rows), np.array(labels))
        # ====== YOUR CODE: ======
        for feature_index in range(rows.shape[1]):
            # sort the feature values
            splits = self.create_feature_splits(rows[:,feature_index])
            for split in splits:
                curr_question = Question(FEATURE_NAMES[feature_index], feature_index, split)
                gain, true_rows, true_labels, false_rows, false_labels = self.partition(rows, labels, curr_question, current_uncertainty)
                if gain >= best_gain and curr_question not in self.used_features:
                    best_gain = gain
        ...
        # ========================
        self.used_features.add(best_question)
    ...
    def build_tree(self, rows, labels):
        ...
        # ====== YOUR CODE: ======
        counts = class_counts(rows, labels)
        all_exist = all(key in counts for key in ['M', 'B'])
        if all_exist == False or len(rows) < self.min_for_pruning:
            return Leaf(rows, labels)
        _, best_question, best_true_rows, best_true_labels, best_false_rows, best_false_labels = self.find_best_split(np.array(rows), np.array(labels))
    ...
    def predict_sample(self, row, node: DecisionNode | Leaf = None):
        ...
        while isinstance(node, DecisionNode):
            curr_question = node.question
            if curr_question.match(row):
                node = node.true_branch
            else:
                node = node.false_branch
        
        predictions = node.predictions
        if predictions.get('M'):
            if predictions.get('B'):
                prediction = 'M' if predictions['M'] > predictions['B'] else 'B'
            else:
                prediction = 'M'
        else:
            prediction = 'B'
